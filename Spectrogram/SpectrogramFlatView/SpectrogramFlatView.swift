...
public struct SpectrogramFlatView: View {
    ...
    private let markerFrequencies: [CGFloat] = [55, 78, 250, 500, 1000, 2000, 4000]
            ...
            ZStack {
                ...
                Canvas { context, size in
                    ...
                    for freq in markerFrequencies {
                        let yPos = size.height * (1 - freq / nyquist)
                        var line = Path()
                        ...
                        let label = Text("\(Int(freq))Hz")
                            .font(.caption2)
                            .foregroundColor(.white.opacity(0.8))
                        context.draw(label, at: CGPoint(x: 4, y: yPos - 8))
                    }
                }
